...
<div class="content">
    ...
    <div id="right">
        ...
        <div class="wrap">
         	<div class="public_index table" style="position: relative;">
                <table>
                	<tr class="line_bold">
                        <th colspan="8">
                    ...
                    <tr class="line<?php if (($this->_foreach['fe_goods']['iteration'] == $this->_foreach['fe_goods']['total'])): ?> last_line<?php endif; ?>">
                        ...
                        <td valign="top"class="align2 bottom-blue" rowspan="<?php echo $this->_var['order']['goods_quantities']; ?>">
                        	<div class="btn-order-status">
                                ...
                                <a href="javascript:;" class="btn-order-status-shipped" ectype="dialog" dialog_title="发货" dialog_id="seller_order_shipped" uri="index.php?app=seller_order&amp;act=shipped&amp;order_id=<?php echo $this->_var['order']['order_id']; ?>&ajax"  dialog_width="400" id="order<?php echo $this->_var['order']['order_id']; ?>_action_shipped"<?php if ($this->_var['order']['status'] != ORDER_ACCEPTED && $this->_var['order']['status'] != COD_ORDER): ?> style="display:none"<?php endif; ?> />发货</a>
...
<script type="text/javascript">
	$(function() {
		...
		if (window.location.href.indexOf('act') > -1 ) {
			$('.order-export').attr('href',window.location.href.replace('act=index','act=export'));
		} else {
			$('.order-export').attr('href',window.location.href + '&act=export');
		}
	})
</script>
