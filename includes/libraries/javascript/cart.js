    ...
    $.getJSON('index.php?app=cart&act=drop&rec_id=' + rec_id, function(result){
        if(result.done){
            ...
            if(result.retval.cart.quantity == 0){
                ...
                credit_span.html(result.retval.credit); //刷新总积分
				...
				$('#cart-number').text(result.retval.cart.quantity);
				// end
            }
        }
    });
...
//          subtotal_span.html(price_format(result.retval.subtotal));
            ...
            credit_span.html(result.retval.credit);
        }
        else{
            //更新失败
            alert(result.msg);
