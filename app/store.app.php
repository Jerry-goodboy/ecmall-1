...
class StoreApp extends StorebaseApp
{
    function index()
    {
		...
		$recommended_goods_array = $this->_get_recommended_goods($id);
		$recommended_goods_number = sizeof($recommended_goods_array);
        /* 取得推荐商品 */
        $this->assign('recommended_goods', $this->_get_recommended_goods($id));
        $this->assign("recommended_goods_number", $recommended_goods_number);
        $this->assign('new_groupbuy', $this->_get_new_groupbuy($id));
        $this->assign('groupbuy_list', $this->_get_new_groupbuy($id));
		$new_goods_array = $this->_get_new_goods($id);
		$new_goods_number = sizeof($new_goods_array);
        /* 取得最新商品 */
        $this->assign('new_goods', $this->_get_new_goods($id));
		$this->assign("new_goods_number", $new_goods_number);
		/* 取得热卖商品 */
		$hot_goods_array = $this->_get_hot_sale_goods($id);
		$hot_goods_number = sizeof($hot_goods_array);
		$this->assign('hot_sale_goods', $this->_get_hot_sale_goods($id));
		$this->assign("hot_goods_number", $hot_goods_number);
        /* 当前位置 */
    ...
    function _get_recommended_goods($id, $num = 12)
    {
        ...
        $goods_list = $goods_mod->find(array(
            ...
            'fields'     => 'goods_name, default_image, price, if_credit, credit_percent',
            'limit'      => $num,
        ));
        foreach ($goods_list as $key => $goods)
        {
            empty($goods['default_image']) && $goods_list[$key]['default_image'] = Conf::get('default_goods_image');
            $goods_list[$key]['credit'] = $goods["price"]*100/CREDIT_RATE;
        }
    ...
    function _assign_searched_goods($id)
    {
        ...
        $order = (empty($_GET["order"])) ? "sales" : $_GET["order"];
        $this->assign("order", $order);
    	$asc = (!isset($_GET["asc"])) ? '1' : ($_GET["asc"]);
        if($asc == 1)
        {
        	$order = $order." asc";
        }
		...
		//var_dump(empty($_GET['order']) || !isset($orders[$_GET['order']]) ? 'add_time desc' : $_GET['order']);
        $page = $this->_get_page(16);
