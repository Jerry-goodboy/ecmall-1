...
class FrontendApp extends ECBaseApp
{
    ...
    {
        $cart = & m('cart');
        if ($cart->get_kinds(SESS_ID, $this->visitor->get('user_id'))) {
    ...
    function _do_wxautologin()
    {
        ...
        if (! empty($user_openid) && ! empty($store_openid) && ! empty($wx_store_id)) {
            $_COOKIE['user_openid'] = $user_openid;
            $_COOKIE['store_openid'] = $store_openid;
            $_COOKIE['wx_store_id'] = $wx_store_id;
    ...
    function _do_wxloginrelation($user_id)
    {
        ...
        if (! empty($_COOKIE['user_openid']) && ! empty($_COOKIE['store_openid']) && ! empty($_COOKIE['wx_store_id'])) {
            $data = array(
                'user_openid' => $_COOKIE['user_openid'],
                'store_openid' => $_COOKIE['store_openid'],
                'store_id' => $_COOKIE['wx_store_id'],
                'user_id' => $user_id
            );
            $wxrelation_mod = & m('wxrelation');
    ...
    function login()
    {
        ...
        if (! IS_POST) {
            ...
            if (Conf::get('captcha_status.login') && base64_decode($_COOKIE['captcha']) != strtolower($_POST['captcha'])) {
    ...
    function _do_login($user_id)
    {
        ...
        /* 更新购物车中的数据 */
        $mod_cart = & m('cart');
        $mod_cart->edit("(user_id = '{$user_id}') AND store_id <> '{$my_store}'", array(
            'user_id' => $user_id
        ));
...
class MallbaseApp extends FrontendApp
{
    ...
    function _run_action()
    {
        ...
        if (! $this->visitor->has_login && in_array(APP, array(
            ...
            header('Location: http://www.hnsjb.cn/web/sellerapply.html');
            return;
        }
        parent::_run_action();
    }
    ...
    {
        parent::_config_view();
        if (ECMALL_WAP == 1 && $GLOBALS["member"] == 0 && $GLOBALS["apply"] == 0) {
...
class MemberbaseApp extends MallbaseApp
{
    ...
    function _run_action()
    {
        ...
        if (! $this->visitor->has_login && ! in_array(ACT, array(
            ...
            if (! IS_AJAX) {
                // header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
                header('Location:http://www.hnsjb.cn/web/sellerlogin.html');
                return;
            } else {
                $this->json_error('login_please');
                return;
    ...
    function _curitem($item)
    {
        ...
        if (! $this->visitor->get('has_store')) {
            ...
            if ($_COOKIE['member_role'] == 'buyer_admin') {
                unset($member_menu['im_seller']);
                $this->assign('member_role', 'buyer_admin');
            } else {
...
class StorebaseApp extends FrontendApp
{
    ...
    function _get_store_kmenusinfo($store_id)
    {
        ...
        $kmenusinfo = $kmenusinfo_mod->find(array(
