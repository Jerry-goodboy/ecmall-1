        ...
        } elseif ($tag{0} == '/') // 结束 tag
{
            ...
            switch ($tag_sel) {
                ...
                case 'cycle':
                    ...
                    if (count($cycle_para) > 1) {
                        $t = array(
                            trim($cycle_para[0]) => trim($cycle_para[1], "'")
                        );
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        return preg_replace_callback($pattern_callback, function ($matches)
        {
    ...
    function url_rewrite($arr)
    {
        ...
        $custom_url = '';
        $tmp = array();
        if (isset($arr['store_id']) && $arr['store_id'] > 0 && isset($arr['app']) && in_array($arr['app'], array(
            'store',
            'goods'
        ))) {
    ...
    function html_checkbox($arr)
    {
        ...
        $out = '';
        foreach ($options as $key => $val) {
            $out .= in_array($key, $checked) ? "<label><input type=\"checkbox\" name=\"$name\" value=\"$key\" checked>&nbsp;{$val}</label>$separator" : "<label><input type=\"checkbox\" name=\"$name\" value=\"$key\">&nbsp;{$val}</label>$separator";
        }
