 ...
 *
 */
class SsoApp extends MallbaseApp
{
    function index(){
    	$system = array(
			'phpsso_appid' => '1',	//应用id	
			//'phpsso_api_url' => 'http://first.lee.com/phpsso_server',	//接口地址
			//'phpsso_auth_key' => '0e0jh3ogydgz3o37uid4giydqg059fhj', //加密密钥
    	    'phpsso_api_url' => 'http://webapp.lvgs.com.cn/phpsso_server',	//接口地址
    	    'phpsso_auth_key' => '0e0jh3ogydgz3o37uid4giydqg059fhj', //加密密钥
			'phpsso_version' => '1', //phpsso版本
			);
		define('APP_LOG',"/data/web/s.hnsjb.cn/sso/error.log");
	 	...
	 	*/
		if ($action == 'synlogin') {
			$this->logResult("logintest");
			$uid = intval($arr['uid']);
			if(!uid) exit(0);
			header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
			$db = & db();
			$sql = "select * from ecm_member where phpsso_uid = '$uid' limit 1";
			$row = $db->query($sql);
		...
		{
			$phpsso_uid = isset($arr['uid']) ? $arr['uid'] : exit('0');
			...
			$reg_time = time();
			$db = &db();
			$sql = "INSERT ignore INTO ecm_member (`phpsso_uid`,`user_name`,`password`,`email`,`last_ip`,`reg_time`)".
			" VALUES ('$phpsso_uid','$user_name','$password','$email','$last_ip','$reg_time')";
			$status =$db->query($sql);
			if($status)exit("1");
			else exit("0");
		}
	...
	function logResult($word='') {
		$fp = fopen("/data/web/s.hnsjb.cn/sso/log.txt","a");
		flock($fp, LOCK_EX) ;
...
//        $params['notify_url'] = "http://s.lvgs.com.cn/mobile/includes/modules/payment/ecmall_notify.php";
