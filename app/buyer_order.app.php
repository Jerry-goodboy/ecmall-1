...
class Buyer_orderApp extends MemberbaseApp
{
    ...
    function view()
    {
        ...
        $order_sql = "select ecm_goods.* from ecm_goods, ecm_order, ecm_order_goods where ecm_order.order_id = '$order_id' and ecm_order.order_id = ecm_order_goods.order_id and ecm_order_goods.goods_id = ecm_goods.goods_id";
        $order_row = $db->query($order_sql);//将订单包含的商品信息取出来
        while($order_test = mysql_fetch_array($order_row))
        {
        	$goods = $order_test;
        }
        $express_code = $order_info['express_name'];
        if($express_code)
        {
        $order_info['express_code'] = $express_code;

        $sql = "select * from ecm_express_info where express_code = '$express_code'";
        $express_result = $db->query($sql);
   	 	while($t_result = mysql_fetch_array($express_result)){
			$mysql_result=$t_result;
		}
        $express_name = $mysql_result['express_name'];
        $order_info['express_name'] = $express_name;
        }
        if (!$order_info)
        {
    ...
    function _get_orders()
    {
        ...
        foreach ($orders as $key1 => $order)
        {
			...
			$shipping_sql = "select * from ecm_order_extm where order_id = '$single_order_id'";//取出当前订单的快递信息
			$shipping_row = $db->query($shipping_sql);
			while($t_result = mysql_fetch_array($shipping_row))
			{
				$shipping_result = $t_result;
			}
			$orders[$key1]["shipping_fee"] = $shipping_result["shipping_fee"];
        }
