...
class Message extends MessageBase
{
    ...
    function report_link($err, $msg, $file, $line)
    {
        if (strncmp($msg, 'MySQL Error[', 12) == 0) {
            $tmp_arr = explode("\n", $msg, 2);
            $tmp_param = strtr($tmp_arr[0], array(
                'MySQL Error[' => 'dberrno=',
                ']: ' => '&dberror='
            ));
            parse_str($tmp_param, $tmp_arr);
            $url = 'http://www.hnsjb.cn/help/faq.php?type=mysql&dberrno=' . $tmp_arr['dberrno'] . '&dberror=' . urlencode($tmp_arr['dberror']);
            ...
            $arr_report = array(
                ...
                'occur_date' => local_date('Y-m-d H:i:s')
            );
