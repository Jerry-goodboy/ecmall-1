...
define('COD_ORDER', 50); // 货到付款，等待卖家发货
/* 特殊文章分类ID */
...
/* 定义积分兑换现金的比例 */
define("CREDIT_RATE", 1); // 1积分兑换多少现金，例如定义为0.5，则1积分可以兑换0.5分钱
        ...
        if (! defined('MODULE')) // 临时处理方案，此处不应对模块进行特殊处理
{
    ...
    function run_module()
    {
        ...
        /* 加载模块配置 */
        Conf::load(array(
            $module_name . '_config' => unserialize($info['module_config'])
        ));
...
EOT;
    ...
    function display($f)
    {
        if ($this->_hook('on_display', array(
            'display_file' => & $f
        ))) {
            return;
        }
        ...
        parent::display($f);
        if ($this->_hook('end_display', array(
            'display_file' => & $f
        ))) {
            return;
        }
    ...
    function _captcha($width, $height)
    {
        import('captcha.lib');
        $word = generate_code();
        $_COOKIE['captcha'] = base64_encode($word);
...
EOT;
    ...
    function _mailto($to, $subject, $message, $priority = MAIL_PRIORITY_LOW)
    {
        ...
        $mails = array();
        $to_emails = is_array($to) ? $to : array(
            $to
        );
    ...
    function _sendmail($is_sync = false)
    {
        if (! $is_sync) {
            ...
            $_COOKIE['ASYNC_SENDMAIL'] = true;
            
            return true;
        } else {
            /* 同步发送邮件，将异步发送的命令去掉 */
            // unset($_COOKIE['ASYNC_SENDMAIL']);
            setcookie('ASYNC_SENDMAIL', "", time() - 100000);
            $model_mailqueue = & m('mailqueue');
    ...
    {
        $script = '';
        if (isset($_COOKIE['ASYNC_SENDMAIL']) && $_COOKIE['ASYNC_SENDMAIL']) {
            /* 需要异步发送 */
...
class BaseVisitor extends Object
{
    ...
    {
        $user_info = unserialize(stripslashes($_COOKIE[$this->_info_key]));
        $user_id = $user_info["user_id"];
        if (! empty($user_id)) {
            $this->info = unserialize(stripslashes($_COOKIE[$this->_info_key]));
            $this->has_login = true;
        } else {
            $this->info = array(
                'user_id' => 0,
    ...
    {
        // $_COOKIE[$this->_info_key] = $user_info;
        setcookie($this->_info_key, serialize($user_info), time() + 3600 * 24 * 365);
    }
        ...
        if ($detail['store_id'] && $detail['state'] != STORE_APPLYING) // 排除申请中的店铺
{
    ...
    function logout()
    {
        // unset($_COOKIE[$this->_info_key]);se
        setcookie($this->_info_key, "", time() - 100000);
    }
