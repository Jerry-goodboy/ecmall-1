...
class WxkeywordModel extends BaseModel {
    ...
    function get_follow_id($user_id = 0) {
        if ($user_id != 0) {
            ...
            $sql = "SELECT kid FROM {$this->table} WHERE isfollow = 1 AND user_id ='" . $_COOKIE['user_info']['user_id'] . "'";
        }
    ...
    function get_mess_id($user_id = 0) {
        if ($user_id != 0) {
            ...
            $sql = "SELECT kid FROM {$this->table} WHERE ismess = 1 AND user_id ='" . $_COOKIE['user_info']['user_id'] . "'";
        }
        return $this->getOne($sql);
    }
