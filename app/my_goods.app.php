...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function add()
    {
        ...
        if (!IS_POST)
        {
             ...
             $this->assign('editor_upload', $this->_build_upload(array(
                 ...
                 'upload_url' => SITE_URL.'index.php?app=swfupload&instance=desc_image',
                 'if_multirow' => 1,
                 'ext_js' => false,
                 'ext_css' => false,
             )));
            ...
            /* 循环出店铺有的配送方式*/ 
        	$sql = "select * from ecm_shipping where store_id = '$store_id'";
			$row = $db->query($sql);
        ...
        else
        {
            ...
            $user_info = unserialize(stripslashes($_COOKIE["user_info"]));
       		$sgrade = $user_info["sgrade"];
			//$sgrade = $_COOKIE['user_info']['sgrade'];//获取店铺级别
			if($sgrade == "3")
			{
				$data['goods']['closed'] = 0; 
			}
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
			...
			$db = & db();
            $store_id = $this->visitor->get('manage_store');
            $check_shipping_sql = "select * from ecm_shipping where store_id = $store_id";
            $check_shipping_row = $db->query($check_shipping_sql);
        	while($t_result = mysql_fetch_array($check_shipping_row)){
				$shipping_result[]=$t_result;
			}
			$select_shipping_sql = "select * from ecm_goods_shipping where goods_id = '$id'";//取出具体商品的配送方式
			$select_shipping_row = $db->query($select_shipping_sql);
        	while($t_result = mysql_fetch_array($select_shipping_row)){
				$select_shipping_result[]=$t_result;
			}
			$this->assign("shipping", $shipping_result);
			$this->assign("select_shipping", $select_shipping_result);
            $this->display('my_goods.form.html');
        }
        else
        {
   ...
   function ajax_col()
   {
    ...
    function _get_post_data($id = 0)
    {
        $goods = array(
        	...
        	'postage_daofu'	   => intval($_POST['postage_daofu']),//邮费到付
        	'if_credit'		   => intval($_POST['if_credit']),//积分商品
        	'credit_percent'   => intval($_POST['credit_percent']),//积分兑换比例，现阶段全为100%
        	'credit_mall'	   => intval($_POST['credit_mall']),//是否为积分商城商品
        	'original_price'   => $_POST["original_price"],//商品原价
        	'shipping_caculation' => $_POST['shipping_caculation'],//邮费计算方式，按件计算还是按重量计算
        );
        ...
        switch ($goods['spec_qty'])
        {
            case 0: // 没有规格
                $specs[intval($_POST['spec_id'])] = array(
                	...
                	'original_price' => $this->_filter_price($_POST['original_price']),
                    'stock' => intval($_POST['stock']),
                	'limit_num' => intval($_POST['limit_num']),//限购数量
                    'sku'      => trim($_POST['sku']),
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            $specs[$key] = array(
                            	...
                            	'limit_num' => intval($_POST['limit_num'][$key]),
    ...
    {
    	$if_credit = $data["goods"]["if_credit"];   	
    	//$credit_mall = $data["goods"]["credit_mall"];
    	if($if_credit)
    	{
    		$data["goods"]["credit_mall"] = 1;
    		$data["goods"]["credit_percent"] = 100;
    	}
        import('image.func');
        import('uploader.lib');
        $db = & db();
        ...
        if ($id > 0)
        {
            ...
            $goods_id = $id;
            $delete_shipping_sql = "delete from ecm_goods_shipping where goods_id='$goods_id'";
            $db->query($delete_shipping_sql);
            foreach($data['goods']['shipping'] as $v)
            {
            	$check_shipping_name_sql = "select * from ecm_shipping where shipping_id = '$v'";
            	$check_shipping_name_row = $db->query($check_shipping_name_sql);
            	while($t_result = mysql_fetch_array($check_shipping_name_row)){
					$shipping_name=$t_result;
				}
				$shipping_fee = $shipping_name['first_price'];
				$shipping_name = $shipping_name['shipping_name'];
         		$insert_shipping_sql = "insert into ecm_goods_shipping (`goods_id`, `shipping_id`, `shipping_name`, `shipping_fee`) values ('$goods_id', '$v', '$shipping_name', '$shipping_fee')";
         		$db->query($insert_shipping_sql);
            }
        ...
        else
        {
            ...
            $shipping_caculation = $data["goods"]["shipping_caculation"];
            foreach($data['goods']['shipping'] as $v)
            {
            	$check_shipping_name_sql = "select * from ecm_shipping where shipping_id = '$v'";
            	$check_shipping_name_row = $db->query($check_shipping_name_sql);
				...
				if($shipping_caculation == 'number')
				{
					...
					$shipping_fee = $shipping_name['first_weight'];
				}
				$shipping_name = $shipping_name['shipping_name'];
            	$insert_shipping_sql = "insert into ecm_goods_shipping (`goods_id`, `shipping_id`, `shipping_name`, `shipping_fee`) values ('$goods_id', '$v', '$shipping_name', '$shipping_fee')";
         		$db->query($insert_shipping_sql);
